...
// Модели
const GEMINI_TEXT_MODEL = 'gemini-2.5-flash';           // Текст
const GEMINI_IMAGE_MODEL = 'imagen-3.0-generate-002';  // Изображения через Imagen 3 API
...
function extractJson(text: string): any | null {
  ...
  try {
    ...
    // Если всё ещё не парсится — пробуем починить обрезанные строки
    const fixed = jsonStr
      .replace(/,\s*([}\]])/g, '$1')  // Убираем trailing commas
      .replace(/\n/g, ' ');            // Заменяем переносы внутри строк на пробелы
    try {
...
async function generateProductData(category: string): Promise<GeneratedProduct | null> {
  const prompt = `Generate product data for e-commerce category "${category}" in Russian.

Requirements:
- name: realistic product name in Russian (20-50 characters)
- slug: latin slug with hyphens (e.g., "mehanicheskaya-klaviatura")
- description: detailed description in Russian, 2-3 sentences highlighting benefits. NO line breaks inside strings.
- price: realistic price in rubles, integer (e.g., 12990)
- specs: 3-4 characteristics as object {key: value}

IMPORTANT: Respond ONLY with a single compact valid JSON object on one line. No markdown, no explanations, no code blocks, no line breaks inside string values.

Format:
{"name":"...","slug":"...","description":"...","price":12345,"specs":{"цвет":"...","материал":"...","вес":"..."}}`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.log(`   ⚠️ Gemini Text API ошибка ${res.status}: ${errData.error?.message || res.statusText}`);
      return null;
    }
    ...
    console.log(`   ⚠️ Ошибка Gemini Text: ${err.message}`);
    return null;
  }
}

// ==================== IMAGEN 3: Генерация изображения ====================
async function generateImage(productName: string, category: string, slug: string): Promise<string | null> {
  ...
  try {
    // Imagen 3 API — отдельный endpoint
    const res = await fetch(
      `${GEMINI_BASE_URL}/models/${GEMINI_IMAGE_MODEL}:predict?key=${GEMINI_API_KEY}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    if (!res.ok) {
      const errData = await res.json().catch(() => ({}));
      console.log(`   ⚠️ Imagen 3 API ошибка ${res.status}: ${errData.error?.message || res.statusText}`);
      return null;
    }

    const data = await res.json() as any;

    // Imagen 3 возвращает predictions с base64
    const predictions = data.predictions;
    if (!predictions || !predictions[0] || !predictions[0].bytesBase64Encoded) {
      console.log('   ⚠️ Нет изображения в ответе Imagen 3');
      return null;
    }
    ...
    console.log(`   ⚠️ Ошибка Imagen 3: ${err.message}`);
    return null;
  }
}
...
async function generateProducts() {
  ...
  console.log(`🎯 Генерация ${PRODUCTS_COUNT} товаров через Google Gemini + Imagen 3...`);
  console.log(`   Текст: ${GEMINI_TEXT_MODEL}`);
  console.log(`   Изображения: ${GEMINI_IMAGE_MODEL}`);
  console.log('');
  ...
  for (let i = 0; i < PRODUCTS_COUNT; i++) {
    ...
    try {
      ...
      if (!imageUrl) {
        console.log('   🖼️ Imagen 3 недоступен, используем Unsplash');
