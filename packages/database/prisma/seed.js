...
async function main() {
    // Categories
    const electronics = await index_1.prisma.category.create({
        data: { name: 'Электроника', slug: 'electronics', description: 'Гаджеты и устройства' }
    });
    const fashion = await index_1.prisma.category.create({
        data: { name: 'Мода', slug: 'fashion', description: 'Одежда и аксессуары' }
    });
    const home = await index_1.prisma.category.create({
        data: { name: 'Дом', slug: 'home', description: 'Товары для дома' }
    });
    const sports = await index_1.prisma.category.create({
        data: { name: 'Спорт', slug: 'sports', description: 'Спортивные товары' }
    });
    const books = await index_1.prisma.category.create({
    ...
    for (const p of products) {
        await index_1.prisma.product.create({ data: p });
    }
    // Test Users
    ...
    for (const a of achievements) {
        await index_1.prisma.achievement.create({ data: a });
    }
    console.log('✅ Seed completed: 5 categories, 10 products, 1 user, 5 achievements');
}
main()
    .catch((e) => { console.error(e); process.exit(1); })
    .finally(async () => { await index_1.prisma.$disconnect(); });
